...
#include <linux/uaccess.h> /* for `get_user()`, `put_user()`, `copy_from_user()`, `copy_to_user()` */
...
#define TOTAL_NUMBER_OF_MINOR_DEVICES 4
static struct cdev my_char_dev[TOTAL_NUMBER_OF_MINOR_DEVICES];
...
static int __init startup(void)
{
    ...
    */
    ...
    //     MINOR(dev_t dev)
    //
    // this variable will be reused several times.
    dev_t dev_number;

    int idx;
    // for simplification, just ignore errors
    // int alloc_ret = -1;
    // int add_ret = -1;
    ...
    //         -x when failed.
    // alloc_ret =
    alloc_chrdev_region(&dev_number, 0, TOTAL_NUMBER_OF_MINOR_DEVICES, DEVICE_NAME);
    // if (alloc_ret)
    // {
    ...
    for (idx = 0; idx < TOTAL_NUMBER_OF_MINOR_DEVICES; idx++)
    {

        cdev_init(&my_char_dev[idx], &fops);
        my_char_dev[idx].owner = THIS_MODULE;

        dev_number = MKDEV(major_number, idx);
        // int cdev_add (struct cdev * p, dev_t dev, unsigned count);
        // add_ret =
        cdev_add(&my_char_dev[idx], dev_number, TOTAL_NUMBER_OF_MINOR_DEVICES);
        // if (add_ret)
        // {
    ...
    for (idx = 0; idx < TOTAL_NUMBER_OF_MINOR_DEVICES; idx++)
    {
        ...
        device_create(cls, NULL, dev_number, NULL, "charone%d", idx);

        pr_info("device file created: /dev/%s%d\n", DEVICE_NAME, idx);
    }
    ...
    //     if (alloc_ret == 0)
    //     {
    //         unregister_chrdev_region(dev_number, TOTAL_NUMBER_OF_MINOR_DEVICES);
    //     }
    //
    //     return -1; // FAILURE
}
...
static void __exit cleanup(void)
{
    ...
    for (idx = 0; idx < TOTAL_NUMBER_OF_MINOR_DEVICES; idx++)
    {
        ...
        pr_info("device file removed: /dev/%s%d\n", DEVICE_NAME, idx);
    }
 ...
 */
static ssize_t device_read(struct file *filp,   /* `file` pointer */
                           char __user *buffer, /* buffer to fill with data, the pointer is in user space, so should not de-ref `buffer` */
                           ...
                           loff_t *offset)
{
    ...
    while (length)
    {
        put_user(CHAR_SEND + minor_number, buffer++);
        length--;
        bytes_read++;
    }

    pr_info("/dev/%s%d: read - complete %d bytes.\n", DEVICE_NAME, minor_number, bytes_read);
